...
from .kv_cache import TabLDMCache
from .inference_config import InferenceConfig


class TabLDM(nn.Module):
    """A Tabular In-Context Learning Foundation Model.

    TabLDM is a transformer-based architecture for in-context learning on tabular data to make
    ...
    This class is the underlying raw PyTorch module for TabLDM. It is not
    intended to be used directly. Instead, use the classes from the top-level
    `tabldm` package such as :class:`tabldm.TabLDMClassifier` or
    :class:`tabldm.TabLDMRegressor` that wrap this class to include the
    ...
    row_rope_interleaved : bool, default=True
        ...
        (the TabLDMv1 variant). If False, uses non-interleaved rotation where the
        embedding is split into first half [0:d//2] and second half [d//2:d]
        (the TabLDMv2 variant). The two variants are not equivalent; checkpoint
    ...
    zero_init : bool, default=True
        ...
        start as identity (TabLDMv1 behavior). Set to False for standard PyTorch
        initialization (used for TabLDMv2 pre-training). Only affects freshly
    ...
    def __init__(
        ...
        self._cache: Optional[TabLDMCache] = None
    ...
    def forward_with_cache(
        ...
        cache: Optional[TabLDMCache] = None,
        cache_mode: str = "kv",
        inference_config: Optional[InferenceConfig] = None,
    ) -> Optional[Tensor]:
        ...
        cache : Optional[TabLDMCache], default=None
        ...
        if store_cache:
            ...
            self._cache = TabLDMCache(train_shape=X_train.shape, num_classes=num_classes)
