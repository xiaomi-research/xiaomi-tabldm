...
try:
    ...
    # Probe whether on-disk JIT caching is usable for this source location.
    # When the package is imported from a read-only location (e.g. via PYTHONPATH
    # from a non-writable dir), numba cannot write its cache and
    # ``@njit(cache=True)`` raises at decoration time when the source file is
    # in a non-writable location. Probe with literal cache=True, then assign
    # _NUMBA_CACHE accordingly so the real functions can opt out of caching.
    try:
        @njit(cache=True)
        def _numba_cache_probe(x):
            return x

        _numba_cache_probe(0)
        _NUMBA_CACHE = True
    except Exception:
        _NUMBA_CACHE = False
