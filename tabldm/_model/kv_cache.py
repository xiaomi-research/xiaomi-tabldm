...
@dataclass
class TabLDMCache:
    """Top-level cache container for the entire TabLDM model.

    This aggregates caches for different components of TabLDM:
    ...
    def slice_batch(self, start: int, end: int) -> TabLDMCache:
    ...
    def to(self, device, dtype=None) -> TabLDMCache:
    ...
    @staticmethod
    def concat(caches: List[TabLDMCache], dim: int = 0) -> TabLDMCache:
        """Concatenate multiple TabLDMCache objects along the batch dimension.

        Parameters
        ----------
        caches : List[TabLDMCache]
